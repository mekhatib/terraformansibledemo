...
resource "awx_credential_machine" "example" {
  ...
  organization = 2
  username = "ubuntu"
  ssh_public_key_data = tls_private_key.example.private_key_pem
...
resource "null_resource" "update_aap_credential" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      API_URL="https://aap.onmi.cloud:8443/api/v2/credentials/"
      DATA='{
        "name": "Terraform Generated Key",
        "description": "SSH Private Key generated by Terraform",
        "organization": 2,
        "credential_type": 1, # Assuming '1' is the ID for Machine credentials
        "inputs": {
          "username": "ubuntu",
          "ssh_key_data": "${self.triggers.private_key}"
        }
      }'
